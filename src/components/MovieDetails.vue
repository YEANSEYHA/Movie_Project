...
<template>
  <div>
      ...
      <div class="container">
        <div class="row align-items-center position-relative">
              ...
              <div class="movie-details-prime">
                <ul>
                  <li class="share">
                    <a href="#"
                      >{{ nbLike
                      }}<i @click="clickLike()" class="fas fa-thumbs-up"></i
                    ></a>
                  </li>
                  <li class="streaming">
                    <h6>Prime Video</h6>
                  ...
                  <li class="watch">
                    <a @click="toggleVideo()" href="#" class="btn popup-video"
          ...
          <div class="movie-details-btn">
            <a v-bind:href="downloadUrl" class="download-btn" download=""
    ...
    <center v-show="showVideo" style="background:rgb(112, 104, 104)">
      <div
        style="position:relative; overflow:hidden; padding-bottom:56.25%; padding-top:10px;  height: 720px ; background:rgb(112, 104, 104)"
      >
        <iframe
          v-bind:src="videoUrl"
          width="1080"
          ...
          scrolling="auto"
          allowfullscreen
          style="background:rgb(112, 104, 104)"
        ></iframe>
      </div>
    </center>
            ...
            <div class="border p-2">
              ...
              <div class="" v-for="review in reviews" :key="review">
                <p class="my-2" style="color: black;">
                  <a style="font-weight: bold;">Commenter:</a>
                  {{ review.comment }}
                </p>
              </div>
            </div>
        ...
        <div class="col-12">
          ...
          <section class="card">
            ...
            <form @submit.prevent="postReport" class="card-body">
              ...
              /><br /><br />
              <textarea
                placeholder="Report the technical problems"
                name="message"
                rows="4"
                cols="100"
                v-model="message"
...
export default {
  ...
  async mounted() {
    let { data } = await axios.get(
      "http://localhost:3000/api/movies/" + this.id
    );

    (this.reviews = data.reviews), (this.title = data.title);
  ...
  methods: {
    async postComment() {
      ...
      await axios
        .post(
          "http://localhost:3000/api/movies/" + this.id + "/reviews",
          postData
        )
        .then((res) => {
    ...
    async postReport() {
      ...
      await axios
        .post("http://localhost:3000/api/movies/reports", postData)
        .then((res) => {
    ...
    async clickLike() {
      await axios
        .post("http://localhost:3000/api/movies/" + this.id + "/like-movie")
        .then((res) => {
    ...
    async toggleVideo() {
      ...
      await axios
        .post("http://localhost:3000/api/movies/" + this.id + "/count-view")
        .then((res) => {
          console.log(res.body);
        });
    },
